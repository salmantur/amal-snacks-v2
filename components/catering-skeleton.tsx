// Instant-render fallback for the Suspense boundary around CateringData in
// app/catering/page.tsx. Mirrors home-skeleton.tsx's header/hero/pill-bar chrome
// (same as EditorialPreview so that part never visibly jumps), but shapes its
// content placeholders after EditorialCateringLanding's actual layout (a 2x2 photo
// grid + tier cards) instead of home-skeleton.tsx's product-grid shape - reusing
// that one here made the /catering navigation look like it briefly rendered the
// wrong page before snapping to the real content.
      ...
      <div className="mx-auto flex min-h-screen max-w-[430px] flex-col" style={{ background: SURFACE }}>
        ...
        <div className="mt-2.5 px-5 pb-6">
          ...
          <div className="mb-8 grid grid-cols-2 gap-2.5">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="aspect-square animate-pulse rounded-2xl" style={{ background: IMG_BG }} />
            ))}
          </div>
          <div className="mb-4 h-4 w-28 animate-pulse rounded" style={{ background: IMG_BG }} />
          <div className="flex flex-col gap-3.5">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-[220px] animate-pulse rounded-[20px]" style={{ background: IMG_BG }} />
            ))}
          </div>
        </div>
      </div>

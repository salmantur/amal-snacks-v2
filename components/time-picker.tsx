...
import { Clock, ChevronDown, ChevronLeft, ChevronRight, CalendarDays } from "lucide-react"
...
function generateSlots(minMinutes: number): { date: string; dayLabel: string; dateLabel: string; slots: string[]; isToday: boolean }[] {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (slots.length > 0) {
      ...
      const dayLabel = isToday ? "اليوم" : isTomorrow ? "غداً" : dayNames[day.getDay()]
...
export function TimePicker({ value, onChange, minMinutes = 0, required = false }: TimePickerProps) {
  ...
  const displayValue = value
    ...
    : isOpen
    ? "أقرب وقت ممكن"
    : "لا يوجد توصيل الآن — اختر موعداً"

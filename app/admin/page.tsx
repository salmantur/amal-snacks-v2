...
import { DAY_NAMES, getSaudiDayDate, getSaudiNowParts, getDayLabel, getDateLabel } from "@/lib/checkout-schedule"
...
// "غدًا", or a weekday name (DAY_NAMES, same order as JS Date.getDay()). We
// bucket orders by that first word directly instead of re-parsing a date out
// of the label - it's already the exact semantic bucket we want, and avoids
// the year ambiguity a day+month label without a year would otherwise create.
function getScheduleBucketKey(order: Order): string {
  const firstWord = order.scheduledTime?.trim().split(/\s+/)[0]

  if (!firstWord || firstWord === "اليوم") return "today"
  if (firstWord === "غدًا") return "tomorrow"
  if (DAY_NAMES.includes(firstWord)) return firstWord
